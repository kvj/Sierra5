...
public class CheckboxPlugin extends DefaultPlugin {
	...
	public int[] getCapabilities() throws RemoteException {
		return new int[] { PluginInfo.PLUGIN_CAN_FORMAT, PluginInfo.PLUGIN_HAVE_MENU, PluginInfo.PLUGIN_HAVE_EDIT_MENU };
	}
	...
	public FormatSpan[] format(int index, Theme theme, Node node, String text, boolean selected) throws RemoteException {
		if ("[X]".equals(text)) {
			// Checked
			return new FormatSpan[] { new FormatSpan(text, new ForegroundColorSpan(theme.c7White)) };
		}
		return new FormatSpan[] { new FormatSpan(text, new StyleSpan(Typeface.BOLD)) };
	}

	@Override
	public MenuItemInfo[] getMenu(int id, Node node) throws RemoteException {
		if (node.can(Node.CAPABILITY_EDIT) && id == -1) {
			// Root menu, text
			Matcher m = Pattern.compile(regexp).matcher(node.text);
			if (m.find()) { // Have checkbox
				return new MenuItemInfo[] { new MenuItemInfo(0, MenuItemInfo.MENU_ITEM_ACTION, "Toggle checkbox") };
			}
		}
		return null;
	}
	...
	public MenuItemInfo[] getEditorMenu(int id, Node node) throws RemoteException {
		return new MenuItemInfo[] { new MenuItemInfo(0, MenuItemInfo.MENU_ITEM_INSERT_TEXT, "Add checkbox") };
	}
	...
	public boolean executeAction(int id, Node node) throws RemoteException {
		...
		if (null != controller.getRootService()) { // Have connection
			boolean result = controller.getRootService().update(node, buffer.toString(), null);
			// Log.i(TAG, "Update result: " + result);
			node.text = buffer.toString();
			return result;
		}
		return false;
	}
